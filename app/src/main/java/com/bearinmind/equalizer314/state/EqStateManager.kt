...
class EqStateManager(
    ...
    companion object {
        /** Fixed hard ceiling the band machinery is sized for (default-frequency table, slot indices); the user-facing cap below can be raised up to this. */
    ...
    // Device output sample rate so biquad coefficients match the rate DynamicsProcessing actually runs at; 48000 fallback if the property is missing/unparsable.
    ...
    // Per-channel editing EQs: CSE off → bothEq on both channels; CSE on → leftEq to ch0, rightEq to ch1, activeChannel picks the editing target.
    ...
    // CSE shared "Both" layer: its OWN curve (flat until edited) applied on top of BOTH channels — final L = leftEq + sharedEq, final R = rightEq + sharedEq (summed by the DP converter overlay).
    ...
    // Per-channel slot layouts: L/R may hold different band counts under CSE, so each side keeps its own list (issue #50); `bandSlots` follows `activeChannel`.
    ...
    // Preamp & auto-gain: with CSE on, each side's preamp (preampLeftDb/preampRightDb) applies to its DP channel; preampGainDb is the single shared preamp when CSE is off.
    var preampGainDb: Float = 0f
    var preampLeftDb: Float = 0f
    var preampRightDb: Float = 0f
    var autoGainEnabled: Boolean = false

    /** CSE "Both" edit view: the graph shows the shared layer that sits on top of both channels; per-band tethering is unaffected. */
    ...
    /** The preamp value belonging to the current view: shared preamp when CSE is off, the active side's preamp otherwise. */
    ...
    /** Write the preamp for the current view. In the Both view the value is applied to BOTH sides (one slider gesture, both channels). */
    ...
    // Limiter — defaults match Wavelet's a6/z.java:105 baseline (1 ms attack, 60 ms release, 10:1 ratio, −2 dB threshold, 0 dB post-gain).
    ...
    fun initEq(graphView: EqGraphView) {
        ...
        // CSE on: restore leftEq/rightEq from their own prefs (fork bothEq when missing); LEFT becomes the editing target.
    ...
    fun initBandSlots() {
        // Rebuild every channel's slot list — a non-active list not matching its own band count desyncs and crashes on a later channel switch + band add.
    ...
    /** Populate [target] with exactly one slot per band in [eq]: use [saved] when it matches the band count, else a sequential 0,1,2,… layout (always valid, never out of range). */
    ...
    /** Trim every channel's EQ to the [MAX_BANDS] cap (issue #31). Called when "Add more EQ bands" is toggled off — bands beyond the original 16 drop highest-first. Returns true if any removed. */
    ...
    /** TV Mode hook (issues #35/#55): fired on every pushEqUpdate BEFORE the isProcessing early-return — a Remote-mode phone must sync to the TV even when its own DP is off. */
    var onEqPushed: (() -> Unit)? = null

    fun pushEqUpdate() {
        // Mirror tethered ("Both"-tagged) band edits between L and R — runs even when not processing so in-memory state is correct for the next save.
        syncBothBands()
        // Shared "Both" layer rides every conversion as an overlay while CSE is on (final channel = channel curve + shared curve).
        ParametricToDpConverter.overlayEq =
            if (eqPrefs.getChannelSideEqEnabled() && sharedEq.getBandCount() > 0) sharedEq else null
        onEqPushed?.invoke()
        if (!isProcessing) return
        val dm = eqService?.dynamicsManager ?: run {
            // Service binding still pending (re-bound async on start): persist and let the service push it, so the sound follows the graph.
            if (EqService.isDpRunning) {
                saveState()
                context.startService(Intent(context, EqService::class.java).setAction(EqService.ACTION_APPLY_PERSISTED_EQ))
            }
            return
        }
        ...
        if (eqPrefs.getChannelSideEqEnabled()) {
            // Per-side preamps ride the per-channel input-gain stage on top of the Channel Side Options offsets; the shared preamp is zeroed so it can't double-apply.
            dm.preampGainDb = 0f
    ...
    /** Coalesce rapid EQ updates (graph drags) into at most one DP write per frame; call [flushEqUpdate] on drag-end. */
    fun pushEqUpdateThrottled() {
        if (!isProcessing) return
        // Issue #61: a live drag freezes the DP cutoff layout and the auto-gain offset — every mid-drag write is then a pure gain update instead of a layout reconfigure + stepped whole-mix duck.
    ...
    /** Cancel any queued throttled update and push now — used at drag-end so the final value commits without a frame of latency (with a fresh layout + auto-gain, see #61). */
    fun flushEqUpdate() {
        if (updatePending) {
    ...
    /** Tag every band in [eq] with [ch] — forked per-channel copies must be independent, else the BOTH sync mirrors L onto R (issue #53). */
    ...
    /** Channel Side EQ switch: on enable, fork the current shared EQ into leftEq/rightEq (identical start) with L as the editing target; on disable, flip back to the "both" EQ. */
    fun setChannelSideEqEnabled(enabled: Boolean) {
        ...
        if (enabled) {
            ...
            // Forked/legacy-restored channels must be independent, not tethered (see tagAllBands). Post-#53 saves keep their own tags (a genuine tether saves BOTH on both sides — safe).
            if (!lOk) tagAllBands(leftEq, ParametricEqualizer.Channel.LEFT)
            if (!rOk) tagAllBands(rightEq, ParametricEqualizer.Channel.RIGHT)
            activeChannel = ActiveChannel.LEFT
            parametricEq = leftEq
            // Slot layouts: a prefs-restored channel uses its own saved slots; a freshly-forked channel inherits the shared (bothEq) layout so the arrangement carries across.
    ...
    /** Switch the active editing channel while Channel Side EQ is on. No-op when CSE is off or the channel is already active. */
    ...
    /** Returns the ParametricEqualizer to apply to ch0 (left) and ch1 (right) respectively. In BOTH mode both channels share the same EQ. */
    ...
    /** The EQ of the channel NOT currently being edited, for the dotted ghost curve (issue #53). Null when Channel Side EQ is off. */
    ...
    /** Shared "Both" layer for graph rendering (non-null with CSE on); the graph skips it when the solid curve is the shared layer itself. */
    fun getGraphOverlayEq(): ParametricEqualizer? =
        if (eqPrefs.getChannelSideEqEnabled() && sharedEq.getBandCount() > 0) sharedEq
        else null

    /** Ghost curves: in an L/R view the other channel; in the Both view both channels, so editing the shared layer shows the resulting L and R outputs move live. */
    ...
    /** Set the active band's channel: BOTH mirrors a synced twin into the other channel, L/R keep it on one side. Returns true if the band left the active channel. */
    ...
    /** Copy [src]'s params into [targetEq] at [slot] (creating the band there if absent), tagged BOTH — the synced twin of a "Both" band. */
    ...
    /** "Untether" a BOTH band: keep it on BOTH channels but make the two sides independent (each retains its position) instead of dropping it from the other channel (issue #53). */
    ...
    /** Create the synced twin for a freshly-added BOTH band (issue #53) — explicit one-time insert; no-op outside CSE or if the band isn't BOTH. */
    ...
    /** Repair tether tags after loading persisted L/R bands (issue #53): a BOTH/BOTH pair with differing params is demoted to independent L/R tags. Runs after slots are rebuilt. */
    ...
    /** Sync every tethered ("Both") band in the active channel to its slot-matched twin in the other channel. Called before each persist/DP push. No-op outside CSE. */
    fun syncBothBands() {
        if (!eqPrefs.getChannelSideEqEnabled() || activeChannel == ActiveChannel.BOTH) return
        ...
        for (i in 0 until parametricEq.getBandCount()) {
            ...
            // Update the existing twin only — inserting here caused runaway band duplication when slot sets differed (issue #53).
            val j = otherSlots.indexOf(slot)
            val other = if (j >= 0) otherEq.getBand(j) else null
            // A tether is bilateral: sync only when the other channel's band at this slot is also BOTH; one-sided tags are demoted to this channel.
    ...
    /** Minimal structure for a preset's band list, shared between the preset-save / preset-load / APO-round-trip paths. */
    data class BandSpec(
        val frequency: Float,
        val gain: Float,
        val q: Double,
    ...
    /** Replace in-memory EQ state from a parsed preset: CSE off → [bothBands] into bothEq; CSE on → [leftBands]/[rightBands] into leftEq/rightEq with LEFT active. Persists the CSE pref. */
    ...
    fun applyPresetEqs(
        ...
        if (cseEnabled) {
            ...
            // Shared "Both" layer from the preset — flat when the preset predates it, so stale layers never leak into a loaded preset (adam's report).
            ...
            // Presets carry no channel tags; default BOTH tags would mirror L over R on the first channel switch (issue #53), so preset-loaded channels are made independent.
            ...
            activeChannel = ActiveChannel.BOTH
            parametricEq = bothEq
            // Wipe stale per-channel prefs so re-enabling CSE forks from the newly-loaded bothEq instead of resurrecting old divergence
    ...
    /** Apply only channel-side-options changes (balance / per-channel preamp) without recomputing the EQ curve. Cheap enough to call on every slider step. */
    ...
    fun doStartEq(animatePower: (Boolean) -> Unit) {
        ...
        dm.limiterThresholdDb = limiterThresholdDb
        dm.limiterPostGainDb = limiterPostGainDb
        // MBC topology must be set BEFORE DP construction so the right number of MBC bands is allocated; per-band params (threshold, ratio, attack…) push AFTER start via applyPersistedMbcConfig.
        ...
        // TV Mode (issues #35/#55): isProcessing just flipped ON — the ONLY reliable signal on the power-button path (EQ_STARTED broadcast fires only on the QS-tile path); sync power state to peer.
        onEqPushed?.invoke()
    }
    ...
    fun saveState() {
        ...
        syncBothBands()
        // Simple mode holds the 10 fixed bands in parametricEq — never write them to the "bands" pref; the advanced EQ is backed up on entering Simple mode and Simple gains persist separately.
    ...
    /** Mirror the active-EQ save so L/R divergence survives restart; no-op when CSE is off. Call alongside eqPrefs.saveState. */
